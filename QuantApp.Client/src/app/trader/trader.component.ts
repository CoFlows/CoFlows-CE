...
export class TraderComponent {
    ...
    private positions2Tree(strategy, expand) {
        ...
        let node : TreeNode = {
            data: {
                ...
                VaR: this.quantapp.formatNumber(strategy.VaR, false)
            },
            children: positions
        };
        ...
        if (strategy.AggregatedOrders != undefined) {
            strategy.AggregatedOrders.forEach((element) => {    
                ...
                if(element.Order.Status == 0 ){
                    ...
                    status = 'Filled: '
                    level = 'Filled@' + this.quantapp.formatNumber(element.Order.ExecutionLevel / element.PointSize,false)
                }
                else if(element.Order.Status == 3){
                    status = 'Filled: '
                    level = 'Booked@' + this.quantapp.formatNumber(element.Order.ExecutionLevel / element.PointSize,false)
                }
    ...
    openPnl(node){
        if(node.children != undefined && node.children.length > 0){
            ...
            return this.quantapp.FindInstrument(node.data.ID).Last * node.data.PointSize * node.data.Position.Unit -  node.data.Position.Strike;
        else
            return 0;
    }
    ...
    dailyPnl(node){
        if(node.children != undefined && node.children.length > 0){
            ...
            return this.quantapp.FindInstrument(node.data.ID).Last * node.data.PointSize * node.data.Position.Unit -  node.data.Position.Strike - node.data.DailyPnlAdjustment;
    ...
    notional(node, abs){
        if(node.children != undefined && node.children.length > 0){
            ...
            return (abs ? Math.abs(this.quantapp.FindInstrument(node.data.ID).Last * node.data.PointSize * node.data.Position.Unit) : this.quantapp.FindInstrument(node.data.ID).Last * node.data.PointSize * node.data.Position.Unit);
        else
            return 0;
    }
    ...
    constructor(private activatedRoute: ActivatedRoute, public quantapp: QuantAppComponent) {
        this.activatedRoute.params.subscribe(params => {
            ...
            var strategy = this.quantapp.FindInstrument(this.MainStrategyID);
            ...
            .subscribe(i => { 
                if(LastPortfolioUpdate != strategy.LastPortfolioUpdate){
                    ...
                    this.quantapp.Get('strategy/portfoliostructure?id=' + this.MainStrategyID,
                    data => {
            ...
            this.quantapp.LinkAction(this.MainStrategyID + '-PortfolioStrategy-MetaData', 
                data => { //Load
            ...
            this.quantapp.Get('strategy/historicalorders?id=' + this.MainStrategyID,
            data => {                
                this.historicalOrders = data                
            });

            this.quantapp.Get('instrument/statistics?id=' + this.MainStrategyID + '&days=20',
            data => {          
                this.statistics = data
            });

            this.quantapp.Get('strategy/analyse?id=' + this.MainStrategyID + '&command=indicators',
            data => {          
                //console.log(data)      
                this.indicators = {}        
                data.forEach(x => {
    ...
    updateChart(node){
        ...
        this.quantapp.Get('instrument/monthlyperformance?id=' + id,
        data => {
            this.monthlyPerformance = data
            //console.log(data)
        })

        this.quantapp.Get('instrument/statistics?id=' + id + '&days=20',
        data => {          
            this.statistics = data
        });

        this.quantapp.Get('instrument/timeseries?id=' + id + '&days=20&spot_type=spot_close',
        data => {
            ...
            if(node.children.length != 0){            
                var chart_config = {
                    ...
                    yAxis: [{
                        ...
                        //     formatter: x => {
                        //         console.log(x)
                        //         return this.quantapp.formatNumber(x.value, true);
                        //     }
                        // }
                    },
                    {
    ...
    onChangeTimeSeries(item){
        ...
        this.updateIndicator(node)
        let id = node.data.ID
        this.quantapp.Get('instrument/intradaytimeseries?id=' + id + '',
        data => {
            ...
            var instrument = this.quantapp.FindInstrument(id)
    ...
    onChangeIndicator(item){
        ...
        this.quantapp.Get('strategy/historicalorders?id=' + node.data.ID,
        data => {
        ...
        this.quantapp.Get('strategy/indicator?id=' + id + '&uid=' + uid + '&iid=' + iid + '&bid=' + bid,
        data => {   
            // if(data.length > 0)
    ...
    submitCode(){
        this.quantapp.Post('strategy/submitcode', 
        {
