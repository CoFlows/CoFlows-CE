...
@Injectable()
export class QuantAppComponent implements  CanActivate  {
    public quantapp_server: string = ''
    ...
    constructor(private http: HttpClient, private router: Router, private msService: MessagingService) {
        ...
        this.quantapp_server = (msService.secure ? 'https' : 'http') + '://' + msService.server + '/'

        this.msService.subscribe(msg => {this.ProcessMessage(msg);})

        let storedToken = localStorage.getItem('CoFlows-QuantAppJWT')
        ...
        interval(500)
        .subscribe(i => { 
            if(QuantAppComponent.UpdateInstruments){
            QuantAppComponent._Instruments.forEach((v, id) => {
                QuantAppComponent.Instruments.get(id).Last = QuantAppComponent._Instruments.get(id).Last
                QuantAppComponent.Instruments.get(id).Bid = QuantAppComponent._Instruments.get(id).Bid
                QuantAppComponent.Instruments.get(id).Ask = QuantAppComponent._Instruments.get(id).Ask
            });
        }})
    ...
    checkLogin(url : string): void {
        ...
        try{
            ...
            if(this.header == null){// || this.quser == null){
                ...
                localStorage.setItem('QuantAppURL', url)
                localStorage.setItem('CoFlows-QuantAppJWT', null)
                this.checking = false
                return
            }
    ...
    oauth(token: string, url: string){
        if(!(token == null || token == "null" || token == "")){
            ...
            localStorage.setItem('CoFlows-QuantAppJWT', token);
    ...
    FindInstrument(id) : any {
        ...
        if (!QuantAppComponent.Instruments.has(id)) {
            this.Subscribe(id);
            ...
            QuantAppComponent.Instruments.set(id, instrument);
            QuantAppComponent._Instruments.set(id, _instrument);            
        }

        return QuantAppComponent.Instruments.get(id);        
    }

    private static _Instruments = new Map<string,any>();

    public static ClearInstruments(){
        QuantAppComponent._Instruments = new Map<string,any>()
        QuantAppComponent.Instruments = new Map<string,any>()
        QuantAppComponent.UpdateInstruments = false
    }
    
    _FindInstrument(id) : any {            
        return QuantAppComponent._Instruments.get(id);        
    }
    ...
    GetData(scope, type, action):any {
        this.CheckUser(() => {
            ...
            this.http.get(this.quantapp_server + '/QuantApp/GetUserData?groupid=' + scope.Group.ID + '&type=' + type, { headers: this.header })
    ...
    Post(url : string, data : any, func : any): void {     
        this.CheckUser(() => {
            this.http.post(this.quantapp_server + url, data, { headers: this.header })
            .toPromise().then(response => {  
                func(response);
            })
        })
    ...
    Get(url : string, func : any): void {        
        this.CheckUser(() => {
            ...
            this.http.get(this.quantapp_server + url, { headers: this.header })
            .toPromise().then(response => {  
                func(response);
            })
        })
