...
export class QAStrategiesComponent {
    ...
    notional(node, abs){
        ...
        node.forEach(element => {
            agg_notional += (abs ? Math.abs(this.quantapp.FindInstrument(element.ID).Last * element.PointSize * element.Position.Unit) : this.quantapp.FindInstrument(element.ID).Last * element.PointSize * element.Position.Unit);
        });
    ...
    totalPnL(rows){
        ...
        rows.forEach(node =>{
            agg_notional += this.quantapp.FindInstrument(node.id).Last - node.dailyAdjustment
        });
    ...
    SetStrategies(strategyIDs){
        this.quantapp.Post('strategy/portfoliolist',strategyIDs,
        data => {
            ...
            data.Strategies.forEach(element => {
                ...
                this.quantapp.FindInstrument(element.ID).IsActive = element.Active
